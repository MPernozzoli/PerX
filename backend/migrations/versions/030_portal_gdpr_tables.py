...
Idempotente: salta tabelle/indici gia' presenti (alcuni ambienti hanno gia'
queste tabelle da un'esecuzione parziale precedente).
"""
...
def upgrade() -> None:
    ...
    if not _has_table("portal_privacy_policies"):
        op.create_table(
            "portal_privacy_policies",
    ...
    _create_index_if_missing("ix_portal_privacy_policies_tenant_id", "portal_privacy_policies", ["tenant_id"])

    # ---- portal_consents ----------------------------------------------
    if not _has_table("portal_consents"):
        op.create_table(
            "portal_consents",
            sa.Column("id", sa.String(), primary_key=True),
            sa.Column("tenant_id", sa.String(), sa.ForeignKey("tenants.id"), nullable=False),
    ...
    _create_index_if_missing("ix_portal_consents_tenant_id", "portal_consents", ["tenant_id"])
    _create_index_if_missing("ix_portal_consents_portal_access_id", "portal_consents", ["portal_access_id"])
    _create_index_if_missing("ix_portal_consents_actor_id", "portal_consents", ["actor_id"])
    _create_index_if_missing("idx_portal_consent_actor", "portal_consents", ["actor_id"])
    _create_index_if_missing("idx_portal_consent_tenant_accepted", "portal_consents", ["tenant_id", "accepted_at"])

    # ---- portal_deletion_requests -------------------------------------
    if not _has_table("portal_deletion_requests"):
        op.create_table(
            "portal_deletion_requests",
            sa.Column("id", sa.String(), primary_key=True),
            sa.Column("tenant_id", sa.String(), sa.ForeignKey("tenants.id"), nullable=False),
    ...
    _create_index_if_missing("ix_portal_deletion_requests_tenant_id", "portal_deletion_requests", ["tenant_id"])
    _create_index_if_missing("ix_portal_deletion_requests_portal_access_id", "portal_deletion_requests", ["portal_access_id"])
    _create_index_if_missing("ix_portal_deletion_requests_actor_id", "portal_deletion_requests", ["actor_id"])
    _create_index_if_missing("ix_portal_deletion_requests_eligible_from", "portal_deletion_requests", ["eligible_from"])
    _create_index_if_missing("ix_portal_deletion_requests_status", "portal_deletion_requests", ["status"])
    _create_index_if_missing("idx_deletion_request_status_eligible", "portal_deletion_requests", ["status", "eligible_from"])

    # ---- portal_notification_prefs ------------------------------------
    if not _has_table("portal_notification_prefs"):
        op.create_table(
            "portal_notification_prefs",
    ...
    _create_index_if_missing("ix_portal_notification_prefs_tenant_id", "portal_notification_prefs", ["tenant_id"])
    ...
    _create_index_if_missing("ix_portal_sessions_tenant_id", "portal_sessions", ["tenant_id"])
    _create_index_if_missing("ix_portal_sessions_portal_access_id", "portal_sessions", ["portal_access_id"])
    _create_index_if_missing("ix_portal_sessions_actor_id", "portal_sessions", ["actor_id"])
    _create_index_if_missing("idx_portal_session_active", "portal_sessions", ["portal_access_id", "revoked_at"])
