...
public actor VaultManager {
    ...
    private var sinistrisPath: String { "\(basePath ?? "")/sinistri" }
    ...
    public func createSinistroFolder(sinistroRef: String) async throws {
        let sinistroPath = "\(sinistrisPath)/\(sinistroRef)"
        let fm = FileManager.default
        ...
        // Crea record nel DB
        let folder = SinistroFolder(sinistroRef: sinistroRef, status: .pending)
    ...
    private func absolutePath(sinistroRef: String, folder: String, filename: String) -> String {
        "\(sinistrisPath)/\(sinistroRef)/\(folder)/\(filename)"
    }
    
    /// Path relativo per un file nel vault
    private func relativePath(sinistroRef: String, folder: String, filename: String) -> String {
        "\(sinistroRef)/\(folder)/\(filename)"
    }
    
    // MARK: - File Operations
    
    /// Lista file di un sinistro
    public func listFiles(sinistroRef: String) async throws -> [VaultFile] {
        return try await DatabaseManager.shared.listVaultFiles(sinistroRef: sinistroRef)
    }
    
    /// Ottiene un file per ID
    public func getFile(id: String) async throws -> (VaultFile, Data) {
        guard let file = try await DatabaseManager.shared.getVaultFile(id: id) else {
            throw VaultError.fileNotFound(id)
        }
        
        let filePath = "\(sinistrisPath)/\(file.relativePath)"
    ...
    public func downloadToTemp(fileId: String) async throws -> URL {
        let (file, data) = try await getFile(id: fileId)
    ...
    public func uploadFile(
        ...
        if !fm.fileExists(atPath: sinistroPath) {
            try await createSinistroFolder(sinistroRef: sinistroRef)
        }
        ...
        var filePath = absolutePath(sinistroRef: sinistroRef, folder: folder, filename: finalFilename)
        var counter = 1
        ...
        while fm.fileExists(atPath: filePath) {
            ...
            filePath = absolutePath(sinistroRef: sinistroRef, folder: folder, filename: finalFilename)
            counter += 1
        }
        ...
        let vaultFile = VaultFile(
            ...
            relativePath: relativePath(sinistroRef: sinistroRef, folder: folder, filename: finalFilename),
        ...
        try await updateFolderStats(sinistroRef: sinistroRef)
    ...
    public func deleteFile(id: String) async throws {
        ...
        try await DatabaseManager.shared.deleteVaultFile(id: id)
        
        // Aggiorna stats cartella
        try await updateFolderStats(sinistroRef: file.sinistroRef)
        
        print("[VaultManager] Deleted: \(file.relativePath)")
    }
    
    /// Sposta un file nella cartella _export (per sync con legacy)
    public func moveToExport(id: String) async throws -> VaultFile {
        guard let file = try await DatabaseManager.shared.getVaultFile(id: id) else {
            throw VaultError.fileNotFound(id)
        }
        
        let (_, data) = try await getFile(id: id)
        
        // Elimina il vecchio
        try await deleteFile(id: id)
    ...
    private func updateFolderStats(sinistroRef: String) async throws {
        let files = try await listFiles(sinistroRef: sinistroRef)
        let totalSize = files.reduce(0) { $0 + $1.size }
        
        var folder = try await DatabaseManager.shared.getSinistroFolder(sinistroRef: sinistroRef)
            ?? SinistroFolder(sinistroRef: sinistroRef)
