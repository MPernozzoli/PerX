...
| Tenant & admin | `routes_tenants.py`, `routes_admin.py` | Multi-tenant nel codice, single-tenant in produzione — vedi [[05-Stato-Sviluppo-e-Roadmap]] |
...
`automation`, `device_token`.

## Multi-tenant e platform admin

- V1: `SINGLE_TENANT_MODE=True`, un solo tenant creato via `scripts/bootstrap_single_tenant.py`.
- L'admin definito da `SINGLE_TENANT_ADMIN_EMAIL` è sia platform admin sia admin tenant.
- Gli utenti standard restano filtrati sul proprio tenant; il supporto multi-tenant resta nel
  codice per uso futuro.
