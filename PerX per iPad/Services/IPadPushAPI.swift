...
/// PerX per iPad implementation of DevicePushAPI, wrapping HubAPIClient.
@MainActor
final class IPadPushAPI: DevicePushAPI {
    private let hub: HubAPIClient

    init(hub: HubAPIClient) {
        self.hub = hub
    }
    ...
    func registerDeviceToken(
        ...
        let _: RegisterResponse = try await hub.cloudPost(
            endpoint: "/api/v1/devices/register", body: body
        )
    }

    func unregisterDeviceToken(token: String) async throws {
        try await hub.cloudDelete(endpoint: "/api/v1/devices/\(token)")
    }

    func mintLiveKitToken(sessionId: String) async throws -> PushLiveKitToken? {
        let resp: LiveKitTokenResponseDTO = try await hub.cloudPost(
            endpoint: "/api/v1/communications/sessions/\(sessionId)/livekit-token",
            body: EmptyBody()
        )
    ...
    func acknowledgeSessionAction(sessionId: String, actionType: String) async throws {
        let _: ActionResponse = try await hub.cloudPost(
            endpoint: "/api/v1/communications/sessions/\(sessionId)/actions",
